...
\title{Accessing biological data in R with semantic web technologies} % see comment on Skype
% another question that might be answered in the text is whether this really is specific for biological data 
% it could just as well be generic
...
The improved expressiveness has advantages in tracking provenance and allowing knowledge to be
more explicitly annotated. In order to use semantic web standards in bioinformatics workflows complementary tools are
needed that can access data in semantic web format or that allow the usage semantic web query approaches. \\
...
% If you make it available on CRAN and not on Bioconductor that also indicates that it is generic. 
% You might want to say so and inicate that your examples are from the biology domain but that the approach
% can be used for any domain
\textbf{Results}. We here present three examples showing how the Resource Description Framework
(RDF) and the SPARQL query standards can be employed in R. The first example takes input on BRCA1 SNPs
...
% Before capatilized acronymes one typically uses "an" not "a" 
% Maybe you do, but it is always nice to describe how you would actually use this for a real use case
% Since it is about semantic web approach you might want to really link data. I.e. first query one resource
% do some statistical analysis on the results and then use the resulting data (e.g. divide the proteins in Uniprot
% in groups using the number of SNPs / code length and see whether the ones with higher numbers share other 
% properties (like are present in OMIM, or belong to certain GO classes). 
% a very real usecase would be to ask ArrayExpress Atlas for all genes related to specific diseases and then check whether
% that correlates with known disease-gene links from DisGeNet. (but that might be a different paper)
% 
% I think it is also important to show how this can be used in combination with BridgeDb mapping to also link resources 
% that are not automatically linked. I think that would make it a lot more valuable. 
% probably increasing usage of both this package and BridgeDb itself.
%
...
% I think you can leave out the line about the subset. Your goal was not to expose all of Jena.
%
\end{abstract}
...
databases~\cite{Belleau2008,Chen2010,Samwald2011,Williams2012,Willighagen2013}. The adoption taking place 
for longer already~\cite{EBIRDFPlatform}, but the pace is now increasing.The recent launch of the
Resource Description Framework (RDF) services at the European Bioinformatics Institute is a good example
of the impact it has on the field.
...
% ideally I would add: 
% and in part by the lack of good tutorials and examples to show actual usage possibilities, which is why we 
% provide some of these here.
...
% I suppose you mean remote Mart's which makes it less general than what you provide here, right?
...
% You write "we" but actually there is only one author.
% The vanHage citation seems to be in a strange place. The line about "compared" is comparing this package, right?
% I think the reference should 
...
% this is a bit vague. How often is "not frequently"? And if it changes, why does it? Same for the actual rrdf package
% I think the libraries change when the community agrees on a change and rrdf does when you want to add functionality
% is that right? If so do you have plans for that?
...
% SO how is the rrdflibs package created? Is there code to do that? If so where is it?
...
that basically just handles triples. Both can be created with the \textit{new.rdf} command:
% this line reads a bit disappointing. You first mention "minimal" ontology support, after that I expect the other 
% one to be more extensive. I would turn that around.
% What happens if people have their own external tripls store already? Can you use that. I assume you can query it 
% (using SPARQL) but not store anything there. Is that correct?
...
% I think this can be clearer. I will normally want to store both?
% And why would I do that in the first place? I cannot save them to an external source. So I probably
% want to work with external data I get usuing rrdf plus internal data I already had. What is the advantage of
% doing that in triple format over doing it from what you already had? That is probably trivial to you but for the
% average R user it might not be.
...
% this sounds like a rather random example to me. Why these specifoc 5 SNPs? Why not get the ones that are most 
% important for breast cancer somehow first and then find those?
